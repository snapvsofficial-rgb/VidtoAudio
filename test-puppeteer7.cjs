...
(async () => {
  ...
  await page.evaluate(async () => {
     try {
       ...
       // Monkey-patch importScripts to capture the error
       workerCode = workerCode.replace('importScripts(r)', 'try { importScripts(r); } catch(err) { console.log("importScripts FAILED WITH:", err.toString()); throw err; }');
