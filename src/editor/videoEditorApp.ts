...
import { getTranslations, buildLocalizedPath } from '../i18n';
...
export function updateEditorLanguage(newLang: SupportedLanguage): void {
  if (activeEditorInstance) {
    activeEditorInstance.updateLanguage(newLang);
  }
}

export function renderVideoEditor(container: HTMLElement, initialLang: SupportedLanguage = 'en'): void {
  if (activeEditorInstance && container.querySelector('#editor-root')) {
    activeEditorInstance.updateLanguage(initialLang);
    return;
  }
  ...
  let t = getTranslations(currentLang).editor;
    ...
    <div id="editor-root" class="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans select-none overflow-hidden pb-16 md:pb-0">
      ...
      <header class="h-14 border-b border-slate-800 bg-slate-900/90 backdrop-blur px-4 flex items-center justify-between z-30 flex-shrink-0">
        <div class="flex items-center gap-3">
          <button id="editor-back-home" class="p-2 rounded-lg bg-slate-800/80 hover:bg-slate-700 text-slate-300 hover:text-white transition-colors flex items-center gap-1.5 text-xs font-semibold" title="${t.backTooltip}">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
            <span id="editor-back-text" class="hidden sm:inline">${t.backBtn}</span>
          </button>
          <div class="flex items-center gap-2 border-l border-slate-800 pl-3">
            ...
            <div>
              ...
              <div class="text-[10px] text-teal-400 font-mono flex items-center gap-1">
                ...
                <span id="editor-offline-badge-text">${t.offlineBadge}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="hidden md:flex items-center gap-2 bg-slate-800/80 border border-slate-700/60 rounded-xl p-1 text-xs font-medium">
          <button data-aspect="16:9" class="aspect-btn px-2.5 py-1 rounded-lg bg-brand-600 text-white font-semibold transition-all">${t.aspect16_9}</button>
          <button data-aspect="9:16" class="aspect-btn px-2.5 py-1 rounded-lg text-slate-300 hover:text-white transition-all">${t.aspect9_16}</button>
          <button data-aspect="1:1" class="aspect-btn px-2.5 py-1 rounded-lg text-slate-300 hover:text-white transition-all">${t.aspect1_1}</button>
          <button data-aspect="4:5" class="aspect-btn px-2.5 py-1 rounded-lg text-slate-300 hover:text-white transition-all">${t.aspect4_5}</button>
        </div>

        <!-- Right: Actions & Export -->
        <div class="flex items-center gap-2">
          <!-- In-Editor Language Switcher -->
          <div class="flex items-center bg-slate-800/90 border border-slate-700/80 rounded-lg p-0.5 text-xs font-semibold">
            <button data-editor-lang-switch="en" class="px-2 py-0.5 rounded transition-colors ${currentLang === 'en' ? 'bg-brand-600 text-white' : 'text-slate-400 hover:text-white'}">EN</button>
            <button data-editor-lang-switch="es" class="px-2 py-0.5 rounded transition-colors ${currentLang === 'es' ? 'bg-brand-600 text-white' : 'text-slate-400 hover:text-white'}">ES</button>
            <button data-editor-lang-switch="fr" class="px-2 py-0.5 rounded transition-colors ${currentLang === 'fr' ? 'bg-brand-600 text-white' : 'text-slate-400 hover:text-white'}">FR</button>
          </div>
          ...
          <button id="editor-demo-modal-open" class="hidden sm:flex items-center gap-1 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-teal-300 border border-teal-500/30 rounded-lg text-xs font-semibold transition-colors">
            ...
            <span id="editor-demo-modal-btn-text">${t.demoModalBtn}</span>
          </button>
          <button id="editor-export-btn" class="px-4 py-1.5 bg-gradient-to-r from-teal-500 to-brand-600 hover:from-teal-400 hover:to-brand-500 text-white font-semibold rounded-lg text-xs sm:text-sm shadow-md shadow-brand-500/20 transition-all flex items-center gap-1.5">
            ...
            <span id="editor-export-btn-text">${t.exportBtn}</span>
          </button>
        </div>
      </header>
        ...
        <div class="flex-grow flex flex-col lg:flex-row overflow-hidden relative">
          ...
          <aside class="hidden lg:flex w-20 flex-col items-center py-4 bg-slate-900 border-r border-slate-800 gap-4 flex-shrink-0 z-10">
            <button data-tab="edit" class="tab-trigger flex flex-col items-center gap-1 text-slate-400 hover:text-white text-[11px] font-medium p-2 rounded-xl transition-all">
              ...
              <span class="tab-label-text">${t.tabEdit}</span>
            </button>
            <button data-tab="media" class="tab-trigger flex flex-col items-center gap-1 text-teal-400 font-medium text-[11px] p-2 rounded-xl transition-all">
              ...
              <span class="tab-label-text">${t.tabMedia}</span>
            </button>
            <button data-tab="audio" class="tab-trigger flex flex-col items-center gap-1 text-slate-400 hover:text-white text-[11px] font-medium p-2 rounded-xl transition-all">
              ...
              <span class="tab-label-text">${t.tabAudio}</span>
            </button>
            <button data-tab="text" class="tab-trigger flex flex-col items-center gap-1 text-slate-400 hover:text-white text-[11px] font-medium p-2 rounded-xl transition-all">
              ...
              <span class="tab-label-text">${t.tabText}</span>
            </button>
            <button data-tab="filters" class="tab-trigger flex flex-col items-center gap-1 text-slate-400 hover:text-white text-[11px] font-medium p-2 rounded-xl transition-all">
              ...
              <span class="tab-label-text">${t.tabFilters}</span>
            </button>
          </aside>
          ...
          <main class="flex-grow bg-slate-950 flex flex-col items-center justify-center p-3 sm:p-6 relative overflow-hidden">
            <div id="preview-aspect-container" class="relative bg-black rounded-2xl shadow-2xl border border-slate-800 overflow-hidden flex items-center justify-center transition-all duration-300 aspect-video max-h-[55vh] w-full max-w-4xl">
              ...
              <div id="video-playback-layer" class="absolute inset-0 w-full h-full flex items-center justify-center">
                ...
                <div id="empty-state-banner" class="flex flex-col items-center justify-center text-center p-6 text-slate-500">
                  ...
                  <h4 id="empty-state-title" class="text-white text-base font-bold mb-1">${t.noVideoLoaded}</h4>
                  <p id="empty-state-desc" class="text-xs text-slate-400 max-w-xs mb-4">${t.noVideoDesc}</p>
                  <div class="flex gap-2">
                    <button id="empty-add-demo-btn" class="px-3.5 py-1.5 bg-brand-600 hover:bg-brand-500 text-white rounded-lg text-xs font-semibold transition-colors flex items-center gap-1.5 shadow">
                      ...
                      <span id="empty-add-demo-text">${t.tryDemoBtn}</span>
                    </button>
                    <button id="empty-add-media-btn" class="px-3.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-lg text-xs font-semibold transition-colors flex items-center gap-1.5">
                      ...
                      <span id="empty-add-media-text">${t.addMediaBtn}</span>
                    </button>
                  </div>
                </div>
              </div>
            ...
            <div class="mt-3 flex items-center gap-4 text-xs font-medium">
              <button id="skip-back-btn" class="p-2 text-slate-400 hover:text-white transition-colors" title="${t.rewindTooltip}">
              ...
              <button id="play-pause-btn" class="w-10 h-10 rounded-full bg-teal-500 hover:bg-teal-400 text-slate-950 flex items-center justify-center font-bold shadow-lg shadow-teal-500/30 transition-transform active:scale-95" title="${t.playPauseTooltip}">
              ...
              <button id="skip-fwd-btn" class="p-2 text-slate-400 hover:text-white transition-colors" title="${t.fastForwardTooltip}">
              ...
              <button id="quick-split-btn" class="p-2 text-slate-400 hover:text-teal-400 transition-colors flex items-center gap-1 ml-2 bg-slate-900 px-2.5 rounded-lg border border-slate-800" title="${t.splitTooltip}">
                ...
                <span id="quick-split-btn-text" class="text-[11px] font-semibold">${t.splitBtn}</span>
              </button>
            </div>
          </main>
        </div>
        ...
        <section class="h-64 sm:h-72 bg-slate-900/95 border-t border-slate-800 flex flex-col flex-shrink-0 relative">
          ...
          <div class="h-9 border-b border-slate-800 bg-slate-900 px-4 flex items-center justify-between text-xs text-slate-400">
            <div class="flex items-center gap-3">
              ...
              <span id="timeline-title-label" class="text-[11px] text-slate-400">${t.timelineHeader}</span>
            </div>

            <div class="flex items-center gap-2">
              <span id="timeline-zoom-label" class="text-[10px] uppercase font-semibold text-slate-500">${t.zoomLabel}</span>
          ...
          <div class="flex-grow flex overflow-hidden relative">
            ...
            <div class="w-24 sm:w-32 bg-slate-900 border-r border-slate-800 flex flex-col flex-shrink-0 z-10 text-[11px] font-medium text-slate-400">
              ...
              <div class="h-16 border-b border-slate-800/80 px-2 flex items-center gap-1.5 bg-slate-900/80">
                ...
                <span id="track-header-video" class="truncate">${t.videoTrack}</span>
              </div>
              <!-- Audio Track Header -->
              <div class="h-16 border-b border-slate-800/80 px-2 flex items-center gap-1.5 bg-slate-900/80">
                ...
                <span id="track-header-audio" class="truncate">${t.audioTrack}</span>
              </div>
              <!-- Text Track Header -->
              <div class="h-16 border-b border-slate-800/80 px-2 flex items-center gap-1.5 bg-slate-900/80">
                ...
                <span id="track-header-text" class="truncate">${t.textTrack}</span>
              </div>
            </div>
      ...
      <nav class="lg:hidden fixed bottom-0 left-0 right-0 h-16 bg-slate-900 border-t border-slate-800 flex items-center justify-around px-2 z-40">
        ...
        <button data-tab="audio" class="tab-trigger flex flex-col items-center gap-0.5 text-slate-400 hover:text-white text-[10px] font-medium py-1">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3"></path></svg>
          <span class="mobile-tab-label">${t.tabAudio}</span>
        </button>
        <button data-tab="text" class="tab-trigger flex flex-col items-center gap-0.5 text-slate-400 hover:text-white text-[10px] font-medium py-1">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 8h10M12 8v11m-4 0h8"></path></svg>
          <span class="mobile-tab-label">${t.tabText}</span>
        </button>
        <button data-tab="filters" class="tab-trigger flex flex-col items-center gap-0.5 text-slate-400 hover:text-white text-[10px] font-medium py-1">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4"></path></svg>
          <span class="mobile-tab-label">${t.tabFilters}</span>
        </button>
      </nav>
      ...
      <div id="start-demo-modal" class="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4 z-50 transition-opacity">
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 max-w-md w-full shadow-2xl relative text-center">
          ...
          <h3 id="start-demo-modal-title" class="text-xl font-bold text-white mb-2">${t.demoModalTitle}</h3>
          <p id="start-demo-modal-desc" class="text-slate-400 text-sm mb-6 leading-relaxed">
            ${t.demoModalDesc}
          </p>
          ...
          <div class="flex flex-col gap-3">
            <button id="modal-try-demo-btn" class="w-full py-3.5 px-4 bg-gradient-to-r from-teal-500 to-brand-600 hover:from-teal-400 hover:to-brand-500 text-white font-bold rounded-xl shadow-lg shadow-teal-500/20 transition-all flex items-center justify-center gap-2">
              ...
              <span id="modal-try-demo-btn-text">${t.loadDemoBtn}</span>
            </button>
            ...
            <label for="modal-file-input" class="w-full py-3.5 px-4 bg-slate-800 hover:bg-slate-750 border border-slate-700 hover:border-slate-600 text-slate-200 font-bold rounded-xl transition-all cursor-pointer flex items-center justify-center gap-2">
              ...
              <span id="modal-upload-btn-text">${t.uploadOwnBtn}</span>
          ...
          <div class="mt-5 pt-4 border-t border-slate-800/80 flex items-center justify-center gap-2 text-xs text-slate-500">
            ...
            <span id="start-demo-privacy-text">${t.privacyNoticeModal}</span>
          </div>
        </div>
      </div>
      ...
      <div id="export-progress-modal" class="hidden fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50">
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 max-w-md w-full text-center shadow-2xl">
          ...
          <h3 id="export-modal-title" class="text-lg font-bold text-white mb-1">${t.exportModalTitle}</h3>
          <p id="export-modal-status" class="text-xs text-slate-400 mb-6">${t.exportModalSubtitle}</p>
          ...
          <div class="flex justify-between text-xs font-mono text-slate-500 mb-6">
            ...
            <span id="export-wasm-badge">${t.exportWasmBadge}</span>
          </div>
          ...
          <div id="export-download-area" class="hidden flex flex-col gap-2">
            <a id="export-download-link" href="#" download="VidToAudio_Project.mp4" class="w-full py-3 bg-teal-500 hover:bg-teal-400 text-slate-950 font-bold rounded-xl shadow-lg transition-colors flex items-center justify-center gap-2">
              ...
              <span id="export-download-btn-text">${t.downloadFinalVideo}</span>
            </a>
            <button id="close-export-modal" class="w-full py-2 text-xs font-semibold text-slate-400 hover:text-white transition-colors">${t.exportDoneBtn}</button>
          </div>
        </div>
      </div>
  ...
  function renderActiveTab() {
    ...
    if (activeTab === 'media') {
      tabContentContainer.innerHTML = `
        <div class="space-y-4">
          <div class="flex items-center justify-between">
            ...
            <span class="text-[11px] text-slate-500">${project.videoClips.length + project.audioClips.length} ${t.mediaFilesCount}</span>
          </div>
          ...
          <label for="drawer-add-media" class="w-full py-4 border-2 border-dashed border-slate-700 hover:border-teal-400 rounded-xl bg-slate-800/40 hover:bg-slate-800/80 transition-all flex flex-col items-center justify-center cursor-pointer group p-3 text-center">
            ...
            <span class="text-xs font-semibold text-slate-200 group-hover:text-white">${t.importBoxTitle}</span>
            <span class="text-[10px] text-slate-500 mt-0.5">${t.importBoxFormats}</span>
            <input id="drawer-add-media" type="file" accept="video/*,audio/*" multiple class="hidden" />
          </label>

          <button id="drawer-try-demo-btn" class="w-full py-2.5 bg-slate-800 hover:bg-slate-755 text-teal-300 border border-teal-500/30 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"></path></svg>
            ${t.loadDemoSampleBtn}
          </button>

          <div class="space-y-2 mt-2">
            <h5 class="text-xs font-semibold text-slate-400 uppercase tracking-wider">${t.clipsInProject}</h5>
      ...
      tabContentContainer.innerHTML = `
        <div class="space-y-4">
          ...
          <div class="space-y-2 pt-2 border-t border-slate-800">
            <span class="text-xs font-semibold text-slate-300">${t.trimClipRange}</span>
            <div>
              <div class="flex justify-between text-[11px] text-slate-400 mb-1">
                <span>${t.startTrimOffset}</span>
        ...
        <div class="space-y-4">
          ...
          ${selectedText ? `
            <div class="p-3 bg-slate-800/80 border border-slate-700 rounded-xl space-y-3">
              <div>
                <label class="block text-xs font-semibold text-slate-300 mb-1">${t.textCaptionLabel}</label>
              ...
              <div class="grid grid-cols-2 gap-2">
                <div>
                  <label class="block text-[11px] text-slate-400 mb-1">${t.stylePresetLabel}</label>
                  <select id="edit-text-style" class="w-full bg-slate-900 border border-slate-700 rounded-lg p-1.5 text-xs text-white">
                    <option value="plain" ${selectedText.style === 'plain' ? 'selected' : ''}>${t.stylePlain}</option>
                    <option value="bubble" ${selectedText.style === 'bubble' ? 'selected' : ''}>${t.styleBubble}</option>
                    <option value="glow" ${selectedText.style === 'glow' ? 'selected' : ''}>${t.styleGlow}</option>
                    <option value="cinema" ${selectedText.style === 'cinema' ? 'selected' : ''}>${t.styleCinema}</option>
                  </select>
                </div>
                <div>
                  <label class="block text-[11px] text-slate-400 mb-1">${t.textColorLabel}</label>
      ...
      const presetLabels: Record<string, string> = {
        normal: t.presetNormal,
        cinematic: t.presetCinematic,
        vintage: t.presetVintage,
        warm: t.presetWarm,
        cool: t.presetCool,
        grayscale: t.presetGrayscale,
        cyberpunk: t.presetCyberpunk
      };
      ...
      tabContentContainer.innerHTML = `
        <div class="space-y-4">
          ...
          <!-- Color Presets -->
          <div>
            <label class="block text-xs font-semibold text-slate-300 mb-2">${t.presetsLabel}</label>
            <div class="grid grid-cols-3 gap-1.5">
              ${['normal', 'cinematic', 'vintage', 'warm', 'cool', 'grayscale', 'cyberpunk'].map(preset => `
                <button data-preset="${preset}" class="preset-btn py-1.5 px-2 rounded-lg text-xs capitalize border ${f.filterPreset === preset ? 'bg-teal-500/20 border-teal-400 text-teal-300 font-bold' : 'bg-slate-800 border-slate-700 text-slate-300'}">${presetLabels[preset] || preset}</button>
              `).join('')}
            </div>
          </div>
  ...
  exportBtn?.addEventListener('click', async () => {
    ...
    try {
      ...
      alert(t.exportFailedAlert + (err?.message || err));
  ...
  // Back to Home Button (Localized)
  container.querySelector('#editor-back-home')?.addEventListener('click', () => {
    const targetPath = buildLocalizedPath('/', currentLang);
  ...
  // Dynamic In-Editor Language Switcher Wiring
  container.querySelectorAll('[data-editor-lang-switch]').forEach(btn => {
    btn.addEventListener('click', () => {
      const chosenLang = btn.getAttribute('data-editor-lang-switch') as SupportedLanguage;
      if (chosenLang && chosenLang !== currentLang) {
        const newPath = buildLocalizedPath('/editor', chosenLang);
        window.history.pushState({}, '', newPath);
        updateLanguage(chosenLang);
        if (typeof (window as any).applyLanguageToUI === 'function') {
          (window as any).applyLanguageToUI(chosenLang);
        }
      }
    });
  });

  // Dynamic Language Update Function (DOM-based, preserves project state)
  function updateLanguage(newLang: SupportedLanguage) {
    currentLang = newLang;
    t = getTranslations(newLang).editor;

    // 1. Top navigation & buttons
    const backText = container.querySelector('#editor-back-text');
    if (backText) backText.textContent = t.backBtn;
    const backBtn = container.querySelector('#editor-back-home');
    if (backBtn) backBtn.setAttribute('title', t.backTooltip);

    const offlineBadge = container.querySelector('#editor-offline-badge-text');
    if (offlineBadge) offlineBadge.textContent = t.offlineBadge;

    const demoModalBtn = container.querySelector('#editor-demo-modal-btn-text');
    if (demoModalBtn) demoModalBtn.textContent = t.demoModalBtn;

    const exportBtnText = container.querySelector('#editor-export-btn-text');
    if (exportBtnText) exportBtnText.textContent = t.exportBtn;

    // 2. Aspect ratio buttons
    container.querySelectorAll('.aspect-btn').forEach(btn => {
      const aspect = btn.getAttribute('data-aspect');
      if (aspect === '16:9') btn.textContent = t.aspect16_9;
      else if (aspect === '9:16') btn.textContent = t.aspect9_16;
      else if (aspect === '1:1') btn.textContent = t.aspect1_1;
      else if (aspect === '4:5') btn.textContent = t.aspect4_5;
    });

    // 3. Editor header language switcher buttons active style
    container.querySelectorAll('[data-editor-lang-switch]').forEach(btn => {
      const langKey = btn.getAttribute('data-editor-lang-switch');
      if (langKey === currentLang) {
        btn.className = 'px-2 py-0.5 rounded transition-colors bg-brand-600 text-white';
      } else {
        btn.className = 'px-2 py-0.5 rounded transition-colors text-slate-400 hover:text-white';
      }
    });

    // 4. Desktop Sidebar tab labels
    const desktopTabMap: Record<string, string> = {
      edit: t.tabEdit,
      media: t.tabMedia,
      audio: t.tabAudio,
      text: t.tabText,
    ...
    container.querySelectorAll('aside.hidden.lg\\:flex .tab-trigger').forEach(btn => {
      const tab = btn.getAttribute('data-tab');
      const label = btn.querySelector('.tab-label-text');
      if (tab && label && desktopTabMap[tab]) {
        label.textContent = desktopTabMap[tab];
      }
    });

    // 5. Mobile bottom nav tab labels
    const mobileTabMap: Record<string, string> = {
      edit: t.tabEditShort,
      media: t.tabMedia,
      audio: t.tabAudio,
      text: t.tabText,
    ...
    container.querySelectorAll('nav.lg\\:hidden .tab-trigger').forEach(btn => {
      const tab = btn.getAttribute('data-tab');
      const label = btn.querySelector('.mobile-tab-label');
      if (tab && label && mobileTabMap[tab]) {
        label.textContent = mobileTabMap[tab];
      }
    });

    // 6. Preview Empty State
    const emptyTitle = container.querySelector('#empty-state-title');
    if (emptyTitle) emptyTitle.textContent = t.noVideoLoaded;
    const emptyDesc = container.querySelector('#empty-state-desc');
    if (emptyDesc) emptyDesc.textContent = t.noVideoDesc;
    const emptyAddDemo = container.querySelector('#empty-add-demo-text');
    if (emptyAddDemo) emptyAddDemo.textContent = t.tryDemoBtn;
    const emptyAddMedia = container.querySelector('#empty-add-media-text');
    if (emptyAddMedia) emptyAddMedia.textContent = t.addMediaBtn;

    // 7. Transport buttons tooltips & labels
    container.querySelector('#skip-back-btn')?.setAttribute('title', t.rewindTooltip);
    container.querySelector('#play-pause-btn')?.setAttribute('title', t.playPauseTooltip);
    container.querySelector('#skip-fwd-btn')?.setAttribute('title', t.fastForwardTooltip);
    container.querySelector('#quick-split-btn')?.setAttribute('title', t.splitTooltip);
    const splitText = container.querySelector('#quick-split-btn-text');
    if (splitText) splitText.textContent = t.splitBtn;

    // 8. Timeline header & Track headers
    const timelineTitle = container.querySelector('#timeline-title-label');
    if (timelineTitle) timelineTitle.textContent = t.timelineHeader;
    const timelineZoom = container.querySelector('#timeline-zoom-label');
    if (timelineZoom) timelineZoom.textContent = t.zoomLabel;

    const trackVideo = container.querySelector('#track-header-video');
    if (trackVideo) trackVideo.textContent = t.videoTrack;
    const trackAudio = container.querySelector('#track-header-audio');
    if (trackAudio) trackAudio.textContent = t.audioTrack;
    const trackText = container.querySelector('#track-header-text');
    if (trackText) trackText.textContent = t.textTrack;

    // 9. Start / Demo Modal
    const demoModalTitle = container.querySelector('#start-demo-modal-title');
    if (demoModalTitle) demoModalTitle.textContent = t.demoModalTitle;
    const demoModalDesc = container.querySelector('#start-demo-modal-desc');
    if (demoModalDesc) demoModalDesc.textContent = t.demoModalDesc;
    const modalTryDemoText = container.querySelector('#modal-try-demo-btn-text');
    if (modalTryDemoText) modalTryDemoText.textContent = t.loadDemoBtn;
    const modalUploadText = container.querySelector('#modal-upload-btn-text');
    if (modalUploadText) modalUploadText.textContent = t.uploadOwnBtn;
    const privacyNotice = container.querySelector('#start-demo-privacy-text');
    if (privacyNotice) privacyNotice.textContent = t.privacyNoticeModal;

    // 10. Export Modal
    const exportModalTitle = container.querySelector('#export-modal-title');
    if (exportModalTitle) exportModalTitle.textContent = t.exportModalTitle;
    const exportWasmBadge = container.querySelector('#export-wasm-badge');
    if (exportWasmBadge) exportWasmBadge.textContent = t.exportWasmBadge;
    const exportDownloadBtnText = container.querySelector('#export-download-btn-text');
    if (exportDownloadBtnText) exportDownloadBtnText.textContent = t.downloadFinalVideo;
    const closeExportBtn = container.querySelector('#close-export-modal');
    if (closeExportBtn) closeExportBtn.textContent = t.exportDoneBtn;

    // 11. Re-render active tab content with new language
    renderActiveTab();
  }

  // Register active editor instance for external language switcher
